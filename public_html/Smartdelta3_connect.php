...
$array_all_m  = array();
  ...
  $file_path3 = "//192.168.119.24/tempBliss_192.168.105.174_spider/smartdelta";
 $files1 = scandir($file_path3);
// $file_path2 = "//192.168.119.24/tempBliss_192.168.105.174_spider/position/".$files1[2];
//setting maximum execution time 
$tmpfname = "//192.168.119.24/tempBliss_192.168.105.174_spider/smartdelta/".$files1[2];
...
for ($row = 2; $row <= $lastRow; $row++) {
    ...
    if ($worksheet->getCell('I' . $row)->getFormattedValue() != "" && $worksheet->getCell('I' . $row)->getFormattedValue() != 0) {
        ...
        $c_name1 = rtrim(ltrim($c_name1));

         $array_all_m[$total_data][0] = $c_name1; //script name
        $name = $worksheet->getCell('C' . $row)->getFormattedValue();
         $array_all_m[$total_data][1] =  ucwords(strtolower($name)); //script name
...
//$date1 = date('Y-m-d',PHPExcel_Shared_Date::ExcelToPHP($worksheet->getCellByColumnAndRow('A' . $row)->getValue()));  // array index 1 
  if ($date1 != "") {
        ...
        $name = $worksheet->getCell('K' . $row)->getFormattedValue();
         $array_all_m[$total_data][7] = 0;  //high
        $name = $worksheet->getCell('L' . $row)->getFormattedValue();
         $array_all_m[$total_data][8] = $name;  //high
        $name = $worksheet->getCell('M' . $row)->getFormattedValue();
       $array_all_m[$total_data][9] = "";  //high
        $name = $worksheet->getCell('N' . $row)->getFormattedValue();
        $array_all_m[$total_data][10] = Round ( $name  );  //high
        ...
        if ($sec_type == 'DERIVATIVES') {
            ...
            if ($op_price != 0 || $op_price != "0") {
                if (count($split) > 2) {
                    ...
                    $table_name_fut = "vol_" . $scrip;
                     $array_all_m[$total_data][9] = Round($strike);  //high
                      //echo $live_price."".$op_price."".$qty;
                     $live_price = get_live_data($strike,$scrip,$date1,$op_type,$con_all);  //high
                    $array_all_m[$total_data][11] = Round(($live_price - $op_price) * $qty  )." ".$live_price;
                     
                   
                     $array_all_m[$total_data][12] = $op_type;
                    ...
                    $sql_close = "SELECT spot,days_of_expire FROM `$table_name_fut` order by date limit 1 ";
               ...
               $margins=($total * 16.7 ) / 100;
                $array_all_m[$total_data][7] = Round($margins);  //high
               // echo  $live_price." ".$qty." ".$margins."<br>";
              /*$intrest=($margins * 1) /100;
               ...
               $pendingdays=$ex_days * $oneday;*/
                ...
                $margins=($live_price * $qty );
                $array_all_m[$total_data][7] = Round($margins );  //high
                     }
                            
                            
                            
                              $iv =    Round(option_implied_volatility($call,$close,$strike,0,$ex_days/365,$live_price) * 100,1);
                             // echo $c_name1."".$call." ".$op." ".$strike." ".$close." ".$ex_days." ".$live_price." ".$iv."<br>";
                            $spot_ten_change = $close + ($close * 0.1);
                           
                            $option_price = Round(BlackScholes($op, $spot_ten_change, $strike, $ex_days / 365, 0, $iv + 8));

                            // $call = "true";
                            $spot_ten_change = $close - ($close * 0.1);

                            $option_price2 = Round(BlackScholes($op, $spot_ten_change, $strike, $ex_days / 365, 0, $iv + 8));
                           
                           $array_all_m[$total_data][13] =  Round(($option_price - $live_price) * $qty); //." ".$option_price;
                             $array_all_m[$total_data][14] = Round(($option_price2 - $live_price) * $qty ); //." ".$option_price;
                    ...
                    $array_all_m[$total_data][11] = Round(($live_price - $op_price) * $qty  );
                   // $array_all_m[$total_data][11] = 
                   $array_all_m[$total_data][12] = "FU";
                    
                   $spot_ten_change = ($live_price * 0.1) * $qty;
                     $array_all_m[$total_data][13] = Round($spot_ten_change );
                  
                      $array_all_m[$total_data][14] = -Round($spot_ten_change );
