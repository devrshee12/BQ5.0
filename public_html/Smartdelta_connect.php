...
for ($row = 2; $row <= $lastRow; $row++) {
    ...
    if ($worksheet->getCell('I' . $row)->getFormattedValue() != "" && $worksheet->getCell('I' . $row)->getFormattedValue() != 0) {
        ...
        $name = $worksheet->getCell('C' . $row)->getFormattedValue();
         $array_all_m[$total_data][1] = $name; //script name
        ...
        $name = $worksheet->getCell('K' . $row)->getFormattedValue();
         $array_all_m[$total_data][7] = $name;  //high
       ...
       $array_all_m[$total_data][9] = $name;  //high
        $name = $worksheet->getCell('N' . $row)->getFormattedValue();
        $array_all_m[$total_data][10] = $name;  //high
        $name = $worksheet->getCell('O' . $row)->getFormattedValue();
        $array_all_m[$total_data][11] = $name;  //high
        $close = 0;
        $ex_days = 0;
        $iv = 0;
        $spot_ten_change = 0;
                ...
                if (count($split) > 2) {
                    ...
                    if ($n_close > 0) {
                        while ($row1 = mysqli_fetch_row($result_close)) {                                     //inserting all fetch value to array  // echo "hello";
                            ...
                            // $iv =    Round(option_implied_volatility($call,$close,$strike,0,$ex_days/365,$op_price) * 100,1);
                           ...
                           $array_all_m[$total_data][13] =  $option_price * $qty; //." ".$option_price;
                             $array_all_m[$total_data][14] = $option_price2 * $qty; //." ".$option_price;
                   ...
                   $spot_ten_change = ($op_price * 0.1) * $qty;
