...
for ($row = 2; $row <= $lastRow; $row++) {
    ...
    if ($worksheet->getCell('I' . $row)->getFormattedValue() != "" && $worksheet->getCell('I' . $row)->getFormattedValue() != 0) {
        ...
        if($c_name1 != $check_name)
        {
            if($check_name != "")
            {
         $array_all_m[$total_data][0] = $check_name; //script name
         $array_all_m[$total_data][1] = ucwords(strtolower($c_name)); //script name
         $array_all_m[$total_data][2] = ""; //script name
         $array_all_m[$total_data][3] = Round($margin_table );
           $array_all_m[$total_data][4] = Round($yesterday_pl );  //high
           $array_all_m[$total_data][5] = Round($current_pl);  //high
           $array_all_m[$total_data][6] = Round($ten_up );  //high
           $array_all_m[$total_data][7] = Round($ten_down );  //high
 ...
 //$date1 = date('Y-m-d',PHPExcel_Shared_Date::ExcelToPHP($worksheet->getCellByColumnAndRow('A' . $row)->getValue()));  // array index 1              
     ...
     //   $margin_table = 0;  //high
        $yesterday_pl = $yesterday_pl + $worksheet->getCell('O' . $row)->getFormattedValue();
        ...
        if ($sec_type == 'DERIVATIVES') {
            ...
            if ($op_price != 0 || $op_price != "0") {
                if (count($split) > 2) {
                    ...
                    $current_pl =  $current_pl + ($live_price - $op_price) * $qty;
                    ...
                    if ($n_close > 0) {
                        while ($row1 = mysqli_fetch_row($result_close)) {                                     //inserting all fetch value to array  // echo "hello";
                            ...
                            if ($qty < 0) {
                                ...
                                $margin_table = $margin_table + Round($margins);  //high
                            ...
                            $ten_up = $ten_up + ($option_price - $live_price) * $qty ; //." ".$option_price;
                            $ten_down = $ten_down + ($option_price2  - $live_price) * $qty; //." ".$option_price;
                    ...
                    $ten_up = $ten_up + Round($spot_ten_change);

                    $ten_down = -Round($spot_ten_change) + $ten_down;
                    $total = $live_price * $qty;

                    $margins = ($total * 16.7) / 100;
                    $margin_table = $margin_table + Round($margins) ;
                    ;  //high
                }
            }
        } else {
