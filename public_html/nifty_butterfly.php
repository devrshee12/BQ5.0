...
//$con=mysqli_connect("192.168.105.179","root","","bliss_vol_all"); 
            $sql6 = "SELECT ATM,ATM_price,options,date,time FROM `all_vol_nifty` WHERE  date = (SELECT date
FROM `vol_nifty` ORDER BY entry_number DESC  LIMIT 0, 1) and spot = (SELECT spot
FROM `vol_nifty` ORDER BY entry_number DESC  LIMIT 0, 1) and delta between 0.01 and 0.99 and options = 'CE'  and days_of_expire > 0";                  //select db
            ...
            $k = 0;
            $strike_diff = array(50, 100, 150, 200, 250, 300, 350, 400, 450, 500); /* strike difference that we have to check in butterfly */
            $lot_size1 = 75;
            $lot_size2 = 150;
            $key1 = $key2 = 0;
            $val1 = $val2 = 0;
...
//echo serialize($profit);
            ...
            foreach ($profit as $key => $value) {
                ...
                if (($total_loss > -1100 || ($max_gain > 1000 && $reward > 3)) && $range > 50 && $data_arr[$f3][1] > 5) {

                    $butterfly_bn[$h][0] = $data_arr[$f1][0] . "&nbsp;   CE    &nbsp;" . "<span style='color:yellow'>+ 75  </span> &nbsp;@ " . $data_arr[$f1][1];
                    $butterfly_bn[$h][1] = $data_arr[$f2][0] . "&nbsp;   CE    &nbsp;" . "<span style='color:red'>- 150  </span> &nbsp;@ " . $data_arr[$f2][1];
                    $butterfly_bn[$h][2] = $data_arr[$f3][0] . "&nbsp;  CE   &nbsp;" . "<span style='color:yellow'>+ 75  </span> &nbsp;@ " . $data_arr[$f3][1];
                    ...
                    if ($butterfly_bn[$h][3] >= 0) {
                        ...
                        if (round(ABS($max_gain / $butterfly_bn[$h][3])) > 1) {
            ...
            $sql6 = "SELECT ATM,ATM_price,options FROM `all_vol_nifty` WHERE  date = (SELECT date
FROM `vol_nifty` ORDER BY entry_number DESC  LIMIT 0, 1) and spot = (SELECT spot
FROM `vol_nifty` ORDER BY entry_number DESC  LIMIT 0, 1) and delta between -0.99 and -0.01 and options = 'PE'  and days_of_expire > 0";                  //select db
...
//echo serialize($profit);
            ...
            foreach ($profit as $key => $value) {
                ...
                $range = ($data_arr[$f3][0] - $data_arr[$f1][0]);
                if (($total_loss > "-1100" || ($max_gain > 2000 && $reward > 3)) && $range > 50 && $data_arr[$f1][1] > 5) {

                    $butterfly_bn[$h][0] = $data_arr[$f1][0] . "&nbsp;   PE    &nbsp;" . "<span style='color:yellow'>+ 75  </span> &nbsp;@ " . $data_arr[$f1][1];
                    $butterfly_bn[$h][1] = $data_arr[$f2][0] . "&nbsp;   PE    &nbsp;" . "<span style='color:red'>- 150  </span> &nbsp;@ " . $data_arr[$f2][1];
                    $butterfly_bn[$h][2] = $data_arr[$f3][0] . "&nbsp;  PE   &nbsp;" . "<span style='color:yellow'>+ 75  </span> &nbsp;@ " . $data_arr[$f3][1];
