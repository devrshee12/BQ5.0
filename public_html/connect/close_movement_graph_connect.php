...
$table_name = ltrim(rtrim($table_name));
	$table_name_fut = strtolower("fut_" . $c_name2);
            ...
            if ($row[1] > 5 && $row[1] != "50.0") {
                ...
                if ($row[1]) {
                   ...
                   // $array_avg[] = Round($row_avg[0], 1);
                      ...
                      $sql_current = "SELECT ((high-low)*100/low) FROM `$table_name_fut` WHERE date = '$row[1]'";		
                  //  echo $sql_current;		
                    $result_current = mysqli_query($con, $sql_current);		
	                 $mov = 0;   
             while ($row1 = mysqli_fetch_row($result_current)) {
                    // $array2[] = $row[4];
                   $mov = Round($row1[0], 1);		
	                    }
                     $array2[] = $mov;
