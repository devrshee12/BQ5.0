...
$search =  $_POST['search'];
    if($search == "")
     $sql = "SELECT * FROM `iv3`   where date = (select date from `iv3` order by id desc limit 1) and time = (select time from `iv3` order by id desc limit 1) order by sector ASC";    
 else {
     if ($search == "NIFTY50") {
      ...
      $sql = "SELECT * FROM `iv3`   where sector like '$search%' and date = (select date from `iv3` order by id desc limit 1) and time = (select time from `iv3` order by id desc limit 1) order by sector ASC";
